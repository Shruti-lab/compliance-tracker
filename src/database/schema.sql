...
CREATE TABLE raw_events (
    ...
    source VARCHAR(50) NOT NULL, -- 'ibm' or 'aws'
    raw_data JSONB NOT NULL,
    received_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_raw_events_source (source),
...
CREATE TABLE normalized_events (
    ...
    event_type VARCHAR(100) NOT NULL, -- 'COS_PUBLIC_EXPOSE', 'IAM_USER_NO_MFA', etc.
    ...
    INDEX idx_normalized_events_status (status),
    INDEX idx_normalized_events_event_type (event_type),
    INDEX idx_normalized_events_timestamp (timestamp),
    INDEX idx_normalized_events_resource_id (resource_id)
);
...
CREATE TABLE violations (
    ...
    framework VARCHAR(100) NOT NULL, -- 'SOC2', 'ISO27001', 'HIPAA', etc.
    ...
    INDEX idx_violations_framework (framework),
...
CREATE TABLE evidence_reports (
    ...
    report_format VARCHAR(50) DEFAULT 'markdown', -- 'markdown', 'json', 'pdf'
    generated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    file_path VARCHAR(500),
    INDEX idx_evidence_reports_violation_id (violation_id),
    INDEX idx_evidence_reports_generated_at (generated_at)
);
